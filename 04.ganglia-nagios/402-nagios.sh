...
function install_nagios() {
    ...
    do
        sed \
            -e "s/FQDN/$n$domain_suffix/g" \
            -e "s/HOSTNAME/$n/g" \
            -e "s/IP/${network_mgmt_nic_map[$n]}/g" \
            nagios/host >> /tmp/hosts.cfg.new
    done
    ...
    for n in ${networks[@]}
    do
        sed -e "s/FQDN/$n$domain_suffix/g" nagios/network_services >> /tmp/services.cfg.new
    done    
    ...
    htpasswd -b -c /etc/nagios/passwd nagiosadmin $nagiosadmin_password
...
EOF
        ...
        echo "=== TRACE MESSAGE ===>>> " $c ": 编辑/etc/nagios/nrpe.cfg配置文件" | tee -a $log_file
        scp nagios/controller_nrpe_commands $c:/tmp >> $log_file
        ssh $c /bin/bash << EOF
...
EOF
    ...
    do
        ...
        ssh $n yum install -y nrpe nagios-plugins* openssl >> $log_file

        ### [所有网络节点] 编辑/etc/nagios/nrpe.cfg配置文件，添加服务端地址
        echo "=== TRACE MESSAGE ===>>> " $n ": 编辑/etc/nagios/nrpe.cfg配置文件" | tee -a $log_file
        ssh $n /bin/bash << EOF
            sed -i -e "s/^allowed_hosts.*/allowed_hosts=127.0.0.1, $controller_0_mgmt_ip/" /etc/nagios/nrpe.cfg
EOF
        ...
        echo "=== TRACE MESSAGE ===>>> " $n ": 编辑/etc/nagios/nrpe.cfg配置文件" | tee -a $log_file
        scp nagios/network_nrpe_commands $n:/tmp >> $log_file
        ssh $n /bin/bash << EOF
            cat /tmp/network_nrpe_commands | tee -a /etc/nagios/nrpe.cfg > /dev/null
EOF
...
EOF
        ...
        echo "=== TRACE MESSAGE ===>>> " $h ": 编辑/etc/nagios/nrpe.cfg配置文件" | tee -a $log_file
        scp nagios/compute_nrpe_commands $h:/tmp >> $log_file
        ssh $h /bin/bash << EOF
...
EOF
    ...
    echo "访问nagios web服务确认安装成功：http://"${controller_mgmt_nic_map[$controller_0]}"/nagios"
}
